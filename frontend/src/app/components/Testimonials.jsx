  ...
  return (
    <section className="bg-white py-10 xs:py-14 sm:py-20 md:py-24">
      <div className="mx-auto max-w-7xl px-3 xs:px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative mx-auto mt-8 xs:mt-10 max-w-2xl sm:max-w-3xl md:max-w-4xl">
          {/* Main testimonial */}
          <div className="relative overflow-hidden rounded-2xl xs:rounded-3xl bg-black p-5 xs:p-8 sm:p-12">
            <Quote className="absolute right-4 top-4 xs:right-8 xs:top-8 h-16 w-16 xs:h-24 xs:w-24 text-white/5" />

            <div className="relative">
              <p className="text-base xs:text-lg leading-relaxed text-white/90 sm:text-xl">
                &quot;{testimonials[currentIndex].quote}&quot;
              </p>

              <div className="mt-6 xs:mt-8 flex items-center gap-3 xs:gap-4">
                <div className="flex h-10 w-10 xs:h-12 xs:w-12 items-center justify-center rounded-full bg-black text-base xs:text-lg font-semibold text-white">
          ...
          {/* Navigation */}
          <div className="mt-6 xs:mt-8 flex items-center justify-center gap-3 xs:gap-4">
            <button
              onClick={prevTestimonial}
              className="flex h-8 w-8 xs:h-10 xs:w-10 items-center justify-center rounded-full bg-black/5 text-black transition-colors hover:bg-black/10"
              ...
              {testimonials.map((_, index) => (
                <button
                  ...
                  className={`h-1.5 xs:h-2 rounded-full transition-all ${
                    index === currentIndex
                      ? "w-5 xs:w-8 bg-black"
                      : "w-1.5 xs:w-2 bg-black/20 hover:bg-black/40"
                  }`}
