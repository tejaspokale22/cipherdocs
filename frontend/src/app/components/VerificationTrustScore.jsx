 ...
 */
export default function VerificationTrustScore({ originalCertificateId, verificationFile }) {
  ...
  useEffect(() => {
    if (verificationFile && !autoCalculated && !loading && !result) {
      ...
      setTimeout(() => {
        execute(() => calculateTrustScore(verificationFile, originalCertificateId));
      }, 500);
    }
  }, [verificationFile, autoCalculated, loading, result, originalCertificateId, execute]);
  ...
  const handleCalculate = async () => {
    ...
    await execute(() => calculateTrustScore(uploadedFile, originalCertificateId));
  };
      ...
      <p className="mb-4 text-sm text-gray-600">
        {verificationFile ? (
          <>
            ...
            Our AI will analyze semantic similarity, structure, and metadata to calculate
            a trust score (0-100).
          </>
        ) : (
          <>
            Upload a document to verify its authenticity against the original certificate.
            Our AI will analyze semantic similarity, structure, and metadata to calculate
            a trust score (0-100).
          </>
        )}
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-lg bg-gradient-to-br from-blue-50 to-purple-50 p-6 text-center">
              ...
              <div className="text-sm text-gray-600">
                ...
                {data.trust_level === "MEDIUM" && "⚠ Document may have minor differences"}
                {data.trust_level === "LOW" && "✗ Document has significant differences"}
              </div>
            </div>
